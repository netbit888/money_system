...
# 清理旧数据
if os.path.exists('snapshots/history.json'):
    os.remove('snapshots/history.json')
...
print()
hist_path = 'snapshots/history.json'
print(f'history.json 存在: {os.path.exists(hist_path)}')
if os.path.exists(hist_path):
    with open(hist_path, 'r', encoding='utf-8') as f:
        data = json.load(f)
    print(f'存盘记录数: {len(data)}')
    if data:
...
print(f'history.json 仍存在: {os.path.exists(hist_path)}')
